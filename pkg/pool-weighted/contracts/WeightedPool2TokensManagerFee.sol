...
contract WeightedPool2TokensManagerFee is
    ...
    WeightedOracleMath
{
    ...
    address feeManager;
    constructor(NewPoolParams memory params, uint256 _managerFeePercentage)
        // Base Pools are expected to be deployed using factories. By using the factory address as the action
        // disambiguator, we make all Pools deployed by the same factory share action identifiers. This allows for
        // simpler management of permtokensissions (such as being able to manage granting the 'set fee percentage' action in
    ...
    {
        managerFeePercentage = _managerFeePercentage;
        feeManager = params.owner;
        ...
        // TODO: do this without infinite approvals. DO NOT DO THIS IN PROD
        params.vault.approveTokensForVault(address(this), tokens);
        ...
        )
    {
        ...
        // ******************** Add manager fees
        uint256 invariantAfterProtocolFees = WeightedMath._calculateInvariant(normalizedWeights, balances);

        uint256[] memory dueManagerFeeAmounts = _getDueProtocolFeeAmounts(
            balances,
            normalizedWeights,
            invariantBeforeJoin,
            invariantAfterProtocolFees,
            protocolSwapFeePercentage
        );

        _mutateAmounts(balances, dueManagerFeeAmounts, FixedPoint.sub);
        _downscaleDownArray(dueManagerFeeAmounts);
        IERC20 feeToken = _maxWeightTokenIndex == 0 ? _token0 : _token1;
        require(feeToken.transferFrom(address(_vault), feeManager, dueManagerFeeAmounts[_maxWeightTokenIndex]));
        ...
        )
    {
        ...
        if (_isNotPaused()) {
            ...
            uint256 invariantBeforeManagerFees = WeightedMath._calculateInvariant(normalizedWeights, balances);
            dueProtocolFeeAmounts = _getDueProtocolFeeAmounts(
                balances,
                normalizedWeights,
                _lastInvariant,
                invariantBeforeManagerFees,
                protocolSwapFeePercentage
            );
